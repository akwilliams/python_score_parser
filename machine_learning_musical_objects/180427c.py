...
#    print(df_6.iloc[:-1,:]['delta_y'].min(),df_6.iloc[:-1,:]['delta_y'].mean(),df_6.iloc[:-1,:]['delta_y'].max())
    ...
    if (maximum-mean)/mean>0.5:
        ...
        df_9=df_0.loc[df_0['y0']>df_8['y'].min()+(minimum*0.25)].copy()
        df_9=df_9.loc[df_9['y1']<df_8['y'].max()-(minimum*0.25)]
        df_9=df_9.loc[df_9['x0']>df_8['x'].min()-20]
        df_9=df_9.loc[df_9['x0']<df_8['x'].max()+20]
        df_9=df_9.loc[df_9['area']>500]
        df_9=df_9.reset_index(drop=True)
        print(df_9)
        for instancess in range(df_9.shape[0]):
            info=df_9.iloc[instancess:,:]
            img_dup=img.copy()
            img_dup_blr=cv2.GaussianBlur(img_dup,(w,h),0)
            template=img[info['y0'].tolist()[0]:info['y1'].tolist()[0],info['x0'].tolist()[0]:info['x1'].tolist()[0]]
            template_blr=cv2.GaussianBlur(template,(w,h),0)
#            template=img[info['y0'].tolist()[0]:info['y1'].tolist()[0],info['x0'].tolist()[0]:info['x1'].tolist()[0]]
            ...
            df_10['kind']='fill_missing'
            df_11=df_6.copy()
            df_11=df_11.append(df_10)
            df_11=df_11.sort_values(by=['y'],ascending=[True])
            df_11['delta_y']=df_11['y'].diff().shift(-1).fillna(0)
#            print(df_11.iloc[:-1,:]['delta_y'].min(),minimum,df_11.iloc[:-1,:]['delta_y'].mean(),mean,df_11.iloc[:-1,:]['delta_y'].max(),maximum)
            ...
            if diff_min < 35 and diff_max > 75:
                df_12=df_11.copy()
                print('this one, instance: ',instancess,df_12)
                m,mm,mmm=df_11.iloc[:-1,:]['delta_y'].min(),df_11.iloc[:-1,:]['delta_y'].mean(),df_11.iloc[:-1,:]['delta_y'].max()
                print((mm-m)/mm,(mmm-mm)/mm)
...
for val in range(1):
    df_0,img=init_img_filter('source/scores/img_'+str(val+63)+'.png')
